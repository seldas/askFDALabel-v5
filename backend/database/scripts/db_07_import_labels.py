...
def parse_spl_zip(zip_path):
    ...
    try:
        ...
        sections = []
        sec_elements = root.findall('.//ns:component/ns:section', NS)
        for sec in sec_elements:
            ...
            sec_title = get_el_text(sec_title_el) if sec_title_el is not None else ""
            
            # Serialize the section XML element for search indexing
            content_xml = ET.tostring(sec, encoding='utf-8').decode('utf-8')
...
def ensure_schema():
    ...
    PGUtils.execute_query("""
        ...
        CREATE INDEX IF NOT EXISTS idx_spl_sections_spl_id ON labeling.spl_sections(spl_id);
...
def _flush_batches(meta_batch, ingr_batch, sect_batch, reload_spl_ids):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # 3. Insert Sections
            if sect_batch:
                sect_cols = ['spl_id', 'loinc_code', 'title', 'content_xml']
                sect_rows = [[d[c] for c in sect_cols] for d in sect_batch]
                sect_sql = f"INSERT INTO labeling.spl_sections ({', '.join(sect_cols)}) VALUES %s"
                execute_values(cur, sect_sql, sect_rows, page_size=500)
