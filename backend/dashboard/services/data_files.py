...
FILE_TYPES = {
    'dailymed': {
        ...
        'relative_dir': 'downloads/DailyMed', 'extensions': {'.zip'}, 'update_type': 'labeling',
    },
    'pharmacologic_class': {
        'label': 'Pharmacologic class indexing', 'filename': 'pharmacologic_class_indexing_spl_files.zip',
        'relative_dir': 'downloads/pharmacologic_class_indexing_spl_files', 'extensions': {'.zip'}, 'update_type': 'epc',
    },
    'meddra': {
        'label': 'MedDRA', 'filename': 'meddra.zip', 'relative_dir': 'downloads/MedDRA',
        'extensions': {'.zip'}, 'update_type': 'meddra',
    },
    'orangebook': {
        'label': 'Orange Book', 'filename': 'EOBZIP.zip', 'relative_dir': 'downloads/OrangeBook',
        'extensions': {'.zip'}, 'update_type': 'orangebook',
    },
    'askdrugtox': {
        'label': 'AskDrugTox update', 'filename': 'askdrugtox_update.xlsx', 'relative_dir': 'downloads',
...
def prepare_for_update(data_dir: str | Path, file_type: str) -> None:
    ...
    if file_type == 'orangebook':
        _safe_extract(source, root / 'downloads' / 'OrangeBook' / 'EOB_Latest')
    elif file_type == 'meddra':
        _safe_extract(source, root / 'downloads' / 'MedDRA' / 'MedDRA_latest')
    elif file_type == 'pharmacologic_class':
        _safe_extract(source, root / 'downloads' / 'pharmacologic_class_indexing_spl_files')
